...
public class BlockBrokenEventHandler {
    ...
    public void decreaseDurability(PlayerEntity player, int numBlocksBroken) {
        ...
        if (activeItemStack.isDamageable()) {
            activeItemStack.damage(numBlocksBroken, player, (p) -> {
                if (originalDurability - numBlocksBroken <= 0) {
    ...
    public void findBlocksOfTheSameType(World world, BlockData startingBlock, PlayerEntity player) {
        ...
        if (!player.isCreative()) {
            decreaseDurability(player, blocksFoundArr.length);
        }
    }
